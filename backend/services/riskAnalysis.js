...
function analyzeRouteRisk(routeName, callback) {
  ...
  `;
  db.all(sql, [`%${routeName.split(' ')[0]}%`], (err, issues) => {
    issues = issues || [];
    let score = 0, reasons = [];
    ...
    callback({ riskLevel, score: Math.round(score), reasons: reasons.length ? [...new Set(reasons)] : ['No active issues'], issueCount: issues.length });
  });
}

function suggestAlternateRoute(currentRoute, callback) {
  const sql = `
  ...
  `;
  db.all(sql, [currentRoute], (err, rows) => {
    rows = rows || [];
    callback(rows.map(r => ({ name: r.route_name, start: r.start_point, end: r.end_point, reason: `Used by ${r.uso} driver(s) with low risk` })));
  });
}
